...
def build_ppflags(d):
    ppflags = (d.getVar("DTC_PPFLAGS") or "").split()

    vardeps = []
    for k in (d.getVar("C2A_DTC_STRINGS") or "").split():
        val = d.getVar(k)
        if val:
            ppflags.append(f"-D{k}=\"{val}\" ")
        vardeps.append(k)
    for k in (d.getVar("C2A_DTC_INTS") or "").split():
        val = d.getVar(k)
        if val:
            ppflags.append(f"-D{k}={val}")
        vardeps.append(k)

    d.appendVarFlag("do_compile", "vardeps", " DTC_PPFLAGS C2A_DTC_STRINGS C2A_DTC_INTS " + " ".join(vardeps))
    bb.warn(f"Using DTC_PPFLAGS: {ppflags}")
    return ppflags
...
def devicetree_compile(dtspath, includes, d):
    ...
    bb.warn("Processing {0} [{1}]".format(dtname, dts))

    # preprocess
    ppargs = d.getVar("BUILD_CPP").split()
    ppargs += build_ppflags(d)
    for i in includes:
        ppargs.append("-I{0}".format(i))
    ppargs += ["-o", "{0}.pp".format(dts), dtspath]
    bb.warn("Running DTB preprocessor: {0}".format(" ".join(ppargs)))
    subprocess.run(ppargs, check = True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    bb.warn("Running DTB compiler: {0}".format(" ".join(dtcargs)))
    subprocess.run(dtcargs, check = True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)


python devicetree_c2a_do_compile() {
    import re
